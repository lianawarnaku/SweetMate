...
export function GlassButton({ children, style, tone = "primary", disabled, ...props }: GlassButtonProps) {
  const colors = useTheme();
  if (tone === "primary") {
    return <AccentButton {...props} disabled={disabled} style={style}>{children}</AccentButton>;
  }
  return (
    <Pressable
      {...props}
      disabled={disabled}
      style={({ pressed }) => [styles.button, { shadowColor: tone === "destructive" ? colors.destructive : colors.accentGlow }, style, disabled && styles.disabled, pressed && !disabled && styles.pressed]}
    >
